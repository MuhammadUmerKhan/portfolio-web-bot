...
- [x] Create a golden dataset of 1 recruiter questions for CI/CD (`evals/data_parser.py`). *Note: Reduced to 1 to avoid Groq 429 rate limit errors during automated testing.*
- [x] Build `evals/eval_runner.py` implementing a strict Two-Phase architecture:
  ...
  - **Phase 2**: Uses DeepEval to grade Faithfulness and Answer Relevancy via a custom Portkey LLM Judge wrapper.
...
- `2026-07-11` — Completed Phase 9 Evaluation (Two-Phase Headless Pipeline). Implemented a robust evaluation suite using DeepEval to measure Faithfulness and Answer Relevancy on a mocked golden dataset. Included custom LLM wrappers using Portkey, handling rate limit pacing with Groq APIs, and updated documentation completely to reflect the 2-metric RAG system.
...
- `2026-07-12` — Completed Phase 12 Deployment & CI/CD. Abandoned Hugging Face Gradio deployments in favor of a universal Dockerized container using a single-stage `uv sync --frozen` approach. Baked BAAI/bge-base-en-v1.5 and FlashRank ONNX models directly into the Docker layers to completely eliminate massive cold-start penalties on serverless platforms. Established a robust GitHub Actions CI/CD pipeline (`.github/workflows/ci.yml`) to automatically execute the DeepEval Phase 1 and 2 scoring scripts on every push to the `main` branch, successfully testing the pipeline with injected Pydantic runtime secrets.

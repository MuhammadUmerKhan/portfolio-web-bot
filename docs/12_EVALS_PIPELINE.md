...
**Result: 5 golden RAG samples + 6 guardrails test cases.**

> ⚠️ **Dataset Size Note**: The dataset was reduced from 15 to 5 questions for CI/CD. The free-tier Groq API `JUDGE_GROQ` key throws `429 Too Many Requests` (Rate Limit Exceeded) and takes over 20 minutes to evaluate 15 questions due to the massive token requirements of DeepEval/RAGAS scoring. A separate `golden_dataset_full.json` preserves all 15 questions for local offline testing.
